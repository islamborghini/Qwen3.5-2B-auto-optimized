...
class Engine:
    ...
    def _commit(self, save, n_acc, keep=None, keep_b=None):
        ...
        Fused-kernel layers ("fused" items) re-run the masked kernel in place; the others use fla with keep-masked g/beta.
        lean: keep-masking and state writes for the fla layers are batched across layers (same arithmetic)."""
        if keep is None:
        ...
        for item in save:
            if item[0] == "fused":   # fused kernel: same inputs, keep mask, in-place state + conv shift by n_acc
                ...
                fla_items.append(item)
        if not fla_items:
            return
        if keep_b is None:
            keep_b = keep.to(fla_items[0][6].dtype)
        if not self.lean or len(fla_items) != self.rec_state.shape[0]:
            for conv_state, rec_state, q, k, v, g, beta, xc_all in fla_items:
            ...
            return
        gs = torch.stack([e[5] for e in fla_items]) * keep                     # [nl,1,T,H] one mul for all layers
        bs = torch.stack([e[6] for e in fla_items]) * keep_b
        sts = []
        for i, (conv_state, rec_state, q, k, v, g, beta, xc_all) in enumerate(fla_items):
        ...
        self.conv_state.copy_(torch.stack([e[7] for e in fla_items]).index_select(3, n_acc + self.arangeC))
